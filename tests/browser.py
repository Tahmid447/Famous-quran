...
report={'scope':'Real application with controlled network fixtures. No claim of live video playback.','checks':[]};blank=base64.b64decode('R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7')
with sync_playwright() as pl:
 ...
 def intercept(r):
  ...
  if 'iframe_api' in u:r.fulfill(content_type='text/javascript',body='window.YT={Player:class{constructor(id,o){setTimeout(()=>o.events.onReady(),1)}destroy(){document.getElementById("fqYouTubeFrame")?.remove()}}};window.onYouTubeIframeAPIReady?.();');return
 ...
 try:
  ...
  page.click('[data-recordings="1"]');page.wait_for_selector('.v3-audio-card');assert page.locator('#v3Title').inner_text().startswith('Qari Ibrahim Idris');assert page.locator('.v3-video-choice').count()==3;assert page.locator('#videoSelect').count()==0;assert 'Al-Ossi' in page.locator('#v3Tracks').inner_text();report['checks'].append('Ibrahim first, requested reciters, unrelated catalogue removed')
  page.click('#v3Start');page.wait_for_selector('#fqYouTubeFrame');frame=page.locator('#fqYouTubeFrame');assert '/K1cA-E6MoBU?' in frame.get_attribute('src');assert 'origin=http%3A%2F%2F127.0.0.1%3A8765' in frame.get_attribute('src');assert frame.get_attribute('referrerpolicy')=='strict-origin-when-cross-origin';assert frame.bounding_box()['height']>=200;report['checks'].append('Visible correct iframe, origin/referrer and controls; fixture playback only')
  ...
  page.click('[data-close="studyDialog"]');page.click('[data-lang="ja"]');assert page.locator('html').get_attribute('lang')=='ja';assert page.locator('[data-lang="hi"]').count()==0;assert page.locator('#themeLabel').inner_text()=='表示設定';page.click('[data-lang="bn"]');assert page.locator('html').get_attribute('lang')=='bn';report['checks'].append('English Bangla Japanese retained; new translated controls')
  page.click('[data-lang="en"]');page.set_viewport_size({'width':390,'height':844});page.click('[data-recordings="1"]');page.wait_for_selector('.v3-audio-card');assert page.locator('#studyDialog').bounding_box()['width']<=390;assert page.evaluate('document.documentElement.scrollWidth<=innerWidth');report['checks'].append('390px viewport without horizontal page overflow');assert not errors,errors;report['checks'].append('No JavaScript errors in regression flows')
 finally:
  report['errors']=errors;(out/'browser-report.json').write_text(json.dumps(report,indent=2));ctx.close()
 # Separate real-browser checks, with no network stubbing; source failures are recorded honestly.
 live={'scope':'Real browser using live public services, local test origin','errors':[]};ctx=browser.new_context(viewport={'width':1440,'height':1000},color_scheme='light');page=ctx.new_page();page.on('pageerror',lambda e:live['errors'].append(str(e)))
 try:
  ...
  page.click('[data-recordings="1"]');page.wait_for_timeout(18000);live['featured']=page.locator('#v3Title').inner_text();live['reciters']=page.locator('#v3Tracks').inner_text();page.screenshot(path=str(out/'actual-recordings-light.png'))
  page.click('#v3Start');page.wait_for_timeout(16000);live['playerStatus']=page.locator('#v3PlayerStatus').inner_text();live['iframeCount']=page.locator('#fqYouTubeFrame').count();page.screenshot(path=str(out/'actual-player-light.png'))
  page.click('[data-v3-mushaf]');page.wait_for_timeout(18000);live['scans']=page.locator('.mushaf-page img').evaluate_all('(xs)=>xs.map(x=>({src:x.src,loaded:x.complete&&x.naturalWidth>0,width:x.naturalWidth}))');page.click('#mushafWide');page.screenshot(path=str(out/'actual-mushaf-light.png'));page.click('#mushafBack');page.click('[data-close="studyDialog"]');page.select_option('#themeMode','dark');page.click('[data-recordings="1"]');page.wait_for_timeout(2000);page.screenshot(path=str(out/'actual-recordings-dark.png'))
