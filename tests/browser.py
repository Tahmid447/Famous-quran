...
with sync_playwright() as pl:
 ...
 try:
  ...
  page.click('[data-open="1"]');page.wait_for_selector('#ayahReciter');assert page.locator('#ayahReciter option').all_text_contents()==['Mishary Rashid Alafasy','Abdul Rahman Al-Sudais','Abdul Rahman Al-Ossi','Yasser Al-Dosari'];page.select_option('#ayahReciter','mp3:ossi');page.wait_for_timeout(100);assert page.locator('[data-play]').count()>0;assert 'complete-surah' in page.locator('#ayahPanel').inner_text().lower();page.evaluate("Q.currentPlayback={kind:'full',rank:1,surah:25,surahName:'Al-Furqan',reciterName:'Abdul Rahman Al-Ossi'};Q.updatePlayback()");assert not page.locator('#persistentPlayer').is_hidden();page.click('[data-close="studyDialog"]');assert not page.locator('#persistentPlayer').is_hidden();report['checks'].append('Requested four reciters; Ossi/Dosari safe full-surah mode; mini-player survives dialog close');
