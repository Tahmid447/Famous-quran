...
with sync_playwright() as pl:
 ...
 def intercept(r):
  ...
  if '/api/recordings?' in u:
   if 'kind=ayah' in u:
    r.fulfill(content_type='application/json',body=json.dumps({'reciter':'Abdul Rahman Al-Ossi' if 'reciter=ossi' in u else 'Yasser Al-Dosari','track':{'url':'https://server11.mp3quran.net/timed/025.mp3'},'timings':[{'ayah':n,'start_time':(n-1)*1000,'end_time':n*1000} for n in range(1,78)]}));return
 ...
 try:
  ...
  page.click('[data-open="1"]');page.wait_for_selector('#ayahReciter');assert page.locator('#ayahReciter option').all_text_contents()==['Mishary Rashid Alafasy','Abdul Rahman Al-Sudais','Abdul Rahman Al-Ossi','Yasser Al-Dosari'];page.select_option('#ayahReciter','mp3:ossi');page.wait_for_timeout(50);assert page.locator('[data-play]').count()>0;page.evaluate("Q.currentPlayback={kind:'ayah',rank:1,surah:25,surahName:'Al-Furqan',ayah:63,reciterName:'Abdul Rahman Al-Ossi'};document.querySelector('#ayahPlayer').dispatchEvent(new Event('play'))");assert not page.locator('#persistentPlayer').is_hidden();page.click('[data-close="studyDialog"]');assert not page.locator('#persistentPlayer').is_hidden();report['checks'].append('Requested four ayah reciters and persistent mini-player survives dialog close');
  page.locator('[data-recordings="1"]').evaluate('(e)=>e.click()');page.wait_for_selector('.v3-audio-card');assert page.locator('#v3Title').inner_text().startswith('Qari Ibrahim Idris');assert page.locator('.v3-video-choice').count()==6;assert page.locator('#videoSelect').count()==0;assert 'Al-Ossi' in page.locator('#v3Tracks').inner_text();report['checks'].append('Ibrahim first, six source videos, requested reciters and no unrelated catalogue')
  page.click('#v3Start');page.wait_for_selector('#fqYouTubeFrame');frame=page.locator('#fqYouTubeFrame');assert '/K1cA-E6MoBU?' in frame.get_attribute('src');assert 'origin=http%3A%2F%2F127.0.0.1%3A8765' in frame.get_attribute('src');assert frame.get_attribute('referrerpolicy')=='strict-origin-when-cross-origin';assert frame.bounding_box()['height']>=200;report['checks'].append('Visible correct iframe, origin/referrer and controls; fixture only')
 ...
 try:
  ...
  page.click('[data-recordings="1"]');page.wait_for_timeout(16000);live['featured']=page.locator('#v3Title').inner_text();live['reciters']=page.locator('#v3Tracks').inner_text();page.screenshot(path=str(out/'actual-recordings-light.png'))
  ...
  for i in range(page.locator('[data-full-play]').count()):
   page.locator('[data-full-play]').nth(i).click();page.wait_for_timeout(3000);audio.append(page.locator('#v3Audio'+str(i)).evaluate('(e)=>({currentTime:e.currentTime,readyState:e.readyState,paused:e.paused,error:e.error?.code||null,src:e.currentSrc})'))
  live['audioPlayback']=audio
  if audio:
   page.locator('[data-full-save]').first.click();page.wait_for_function("!document.querySelector('[data-full-save]').disabled",timeout=125000);live['audioSave']=page.locator('#v3AudioStatus0').inner_text();ctx.set_offline(True);page.locator('[data-full-play]').first.click();page.wait_for_timeout(3000);live['savedAudioOffline']=page.locator('#v3Audio0').evaluate('(e)=>({src:e.currentSrc,time:e.currentTime,paused:e.paused,error:e.error?.code||null})');ctx.set_offline(False)
